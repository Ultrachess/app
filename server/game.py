...
class Game:
    ...
    def addPlayer(self, timestamp, player):
        try:
            ...
            hasFunds = (
                deps.accountManager.getBalance(address, self.token) >= self.wagerAmount
            )
            logger.info(
                "cannAdd:"
                + str(canAdd)
    ...
    def getState(self): 
        return {
            ...
            "wagering": deps.betManager.games[self.id]
