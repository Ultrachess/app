      ...
      variableDefinitions: [
        {
          ...
          type: {
            kind: "NamedType",
            name: { kind: "Name", value: "NoticeKeys" },
          },
        },
      ],
      selectionSet: {
        ...
        selections: [
          {
            ...
            selectionSet: {
              ...
              selections: [
                ...
                { kind: "Field", name: { kind: "Name", value: "epoch_index" } },
                { kind: "Field", name: { kind: "Name", value: "input_index" } },
                {
                  kind: "Field",
                  name: { kind: "Name", value: "notice_index" },
                },
...
export const gameSlice = createSlice({
  ...
  reducers: {
        ...
        if (success) {
          switch (operation) {
            ...
            case "join":
              ...
              state.games.forEach((value, index) => {
                if (id == value.id) state.games[index].players.push(player);
              });
              break;
            case "leave":
              break;
            case "move":
              ...
              if (game) {
                ...
                state.games.forEach((value, index) => {
                  if (game.id == value.id) state.games[index] == game;
                });
              }
    ...
    removeGame: (state, action) => {
      ...
      state.games.forEach((value, index) => {
        if (id == value.id) state.games.splice(index, 1);
      });
    },
  },
});
...
export const initContracts = (signer, chainId) => async (dispatch) => {
  ...
  const contracts =
    contract && contract.InputFacet && contract.ERC20PortalFacet
      ? contract
      : CONTRACTS.localhost;
...
export const depositErc20 =
  (signer, tokenAddress, tokenAmount) => async (dispatch) => {
    try {
      let overrides = {
        gasLimit: 30000000,
        gasPrice: ethers.utils.parseUnits("9.0", "gwei"),
      ...
      const tx = await erc20PortalContract.erc20Deposit(
        tokenAddress,
        erc20Amount,
        "0x"
      );
...
export const createBotGame =
  (botId1, botId2, matchCount, wagerAmount) => async (dispatch) => {
    try {
      let overrides = {
        gasLimit: 30000000,
        gasPrice: ethers.utils.parseUnits("9.0", "gwei"),
      ...
      const erc20WagerAmount = ethers.BigNumber.from(
        ethers.utils.parseUnits(wagerAmount)
      );
