...
export function shouldCheckTransaction(lastBlockNumber: number, tx: Transaction){
    // should not check if transaction is already confirms
    if(tx.receipt) return false
    //should check since lastChecked block number is not added
    if(!tx.lastCheckedBlockNumber) return true

    const blocksPassedSinceLastCheck = lastBlockNumber - tx.lastCheckedBlockNumber
...
export function TransactionUpdater() {
    ...
    useEffect(() => {
        ...
        const cancels = Object.keys(pendingTransactions)
            .filter((hash) => shouldCheckTransaction(lastBlockNumber, pendingTransactions[hash]))
            .map((hash) => {
