...
export default ({
  ...
  const pots1 = pots ? pots[p1] ?? 0 : 0;
  const pots2 = pots ? pots[p2] ?? 0 : 0;
    ...
    ? betsArray.map((bet, index) => {
        ...
        return (
          <Text key={index}>
            ...
            <AssetDisplay balance={amount} tokenAddress={tokenAddress} /> on{" "}
            <Address value={winningId} /> at <Date current={timestamp} />
          </Text>
        );
      })
  ...
  return (
    <Flex css={{ flexDirection: "column", gap: 2 }}>
      ...
      <Flex css={{ justifyContent: "space-between" }}>
        ...
        <Flex css={{ flexDirection: "column", gap: 1 }}>
          {closed ? (
            <Text faded>Wagering Closed</Text>
          ) : (
            <Text faded>
              Wagering closes on <Date current={openTime + duration} />
            </Text>
          )}
          <ModalPlaceBet
            gameId={gameId}
            triggerElement={<Button disabled={closed}>Place Bet</Button>}
          />
        </Flex>
      </Flex>
      <List items={betItems} />
