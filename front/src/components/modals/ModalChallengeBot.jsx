...
export default ({ visible, closeHandler, botId }) => {
  ...
  const onChallengerBotValueChange = (newValue) =>
    setChallengerBot(newValue.value);
  ...
  const botList = useMemo(() => {
    return [account]
      ...
      .map((val) => {
        //console.log(val)
        if (isAddress(val)) return { value: val, label: "You" };
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        <Select options={botList} onChange={onChallengerBotValueChange} />
        <Row>
          <Input
            clearable
            bordered
