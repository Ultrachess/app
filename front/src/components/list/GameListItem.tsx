...
export default ({ game }: { game: Game }) => {
  ...
  const joinable =
    game?.players?.length < 2 && !game.isEnd && !game.players.includes(account);
  ...
  return (
    <Flex css={{ gap: 50 }}>
      ...
      <Flex css={{ flexDirection: "column", gap: 2 }}>
        ...
        <Flex css={{ gap: 1 }}>
          {p2 ? (
            <Address value={p2} hoverable={true} />
          ) : (
            <Text faded>waiting for player</Text>
          )}
      ...
      <Flex css={{ flexDirection: "column", gap: 2 }}>
        ...
        <Flex css={{ gap: 1 }}>
          ...
          <DotIcon
            color={completed ? "blue" : bettingIsClosed ? "green" : "red"}
          />
        </Flex>
      </Flex>
      <ModalPlaceBet
        triggerElement={
          <Button disabled={bettingIsClosed}>
            Bet&nbsp;&nbsp; {bettingIsClosed && <LockClosedIcon />}
          </Button>
        }
        gameId={id}
      />
